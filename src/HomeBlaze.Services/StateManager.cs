...
namespace HomeBlaze.Services
{
    public class StateManager : BackgroundService, IDisposable, IStateManager
    {
        ...
        public StateManager(IEventManager eventManager, IBlobContainer blobContainer, ILogger<StateManager> logger)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    lock (this)
                    {
                        groups = queue.GroupBy(q => q.Item1).ToArray();
                        queue = new List<(string, string)>();
                    }
