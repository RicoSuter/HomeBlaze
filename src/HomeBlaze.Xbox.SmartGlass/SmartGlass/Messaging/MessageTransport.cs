...
namespace SmartGlass.Messaging
{
    internal class MessageTransport : IDisposable, IMessageTransport<IMessage>
    {
        ...
        public MessageTransport(string addressOrHostname, CryptoContext crypto)
        {
            ...
            Task.Run(() =>
            {
                while (!_receiveQueue.IsCompleted && !_cancellationTokenSource.Token.IsCancellationRequested)
                {
                    try
                    {
                        if (_receiveQueue.TryTake(out var message, 1000, _cancellationTokenSource.Token))
                        {
