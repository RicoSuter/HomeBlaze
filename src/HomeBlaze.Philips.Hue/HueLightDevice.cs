...
namespace HomeBlaze.Philips.Hue
{
    public class HueLightDevice : IThing, IIconProvider, ILastUpdatedProvider,
        IConnectedDevice, IDimmerLightbulb, IColorLightbulb, IColorTemperatureLightbulb, IPowerConsumptionSensor
    {
        ...
        public string Title => $"{_light.Name} " +
            $"({string.Join(", ", new string?[] { Color != null ? "Color" : null, ColorTemperature.HasValue ? "Temperature" : null, Brightness.HasValue ? "Dimmable" : null, IsOnOffLight ? "On/Off" : null, ModelId }.Where(e => e != null))})";

        public string IconName => "fas fa-lightbulb";

        public Color IconColor => IsLightOn == true ? MudBlazor.Color.Warning : MudBlazor.Color.Default;
        ...
        [Abstractions.Attributes.State]
        public bool? IsLightOn => _light?.State.On;
        ...
        public decimal? PowerConsumption =>
            IsLightOn == true ? (_light?.ModelId) switch
            {
                "LWA001" => 9m,
                "LWA011" => 9m,
                "LWA017" => 9.5m,
            ...
            IsLightOn == false && IsConnected ? 0.5m :
            null;
        ...
        public decimal? Lumen =>
            ...
            IsLightOn == false && IsConnected ? 0m :
            null;
        ...
        public async Task DimmAsync(decimal brightness, CancellationToken cancellationToken)
        {
            ...
            var turnOffAfterChange = IsLightOn != true; // hack: needed to change brightness without turning on the lights
        ...
        {
            var newColorTemperature = (int?)(_light.Capabilities?.Control?.ColorTemperature.Min +
                colorTemperature * (_light.Capabilities?.Control?.ColorTemperature.Max - _light.Capabilities?.Control?.ColorTemperature.Min));
